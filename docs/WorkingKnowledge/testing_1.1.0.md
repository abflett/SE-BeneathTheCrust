...
Working Knowledge `1.1.0` migrates the progress overlay to Rich HUD Master and adds graphical player and administrator settings. Release preparation is underway; the detailed checklist below is not a claim that every scenario has been tested.

## Release preparation status — 2026-09-09

- Maintainer confirmed the revised panel looks and behaves correctly after the latest HUD suppression and section-layout changes.
- Release validation passed for `1.1.0`: script compilation, 19 XML files, thumbnail and icons, built-in catalog parity, and layer generator/priority/fallback checks.
- All 34 settings checks passed, covering numeric input, draft apply/discard, typing delay, and configuration serialization.
- Version metadata already specifies `1.1.0`; the changelog is consolidated for publication and the Workshop description already names Rich HUD Master.
- Before publishing, confirm the hosted/dedicated, save/reload, and unload cases below that have not been individually recorded as completed. Replace Workshop required item Text HUD API (`758597413`) with Rich HUD Master (`1965654081`).
- Publication remains pending. This preparation does not upload the mod, change live Workshop dependencies, create a release tag, or publish the separate Layer Toolkit archive generated by validation.
