...
Working Knowledge deliberately bundles 18 current vanilla `.dds` icons and three custom data icons under `Textures/GUI/Icons/`. A dedicated-server installation does not include the normal client UI texture library, but it still validates mod-relative icon paths in loaded definitions. Supplying every referenced file in the mod removes the resulting 138 false missing-texture errors.

The vanilla set consists of the two terminal icons used by Working Knowledge's vanilla-derived terminal blocks and 16 cube icons used by the generated variant-group splits in `BlockVariantGroups_Research.sbc`. Keep their relative paths unchanged while they remain vanilla copies. The custom set uses the vanilla Datapad casing as a shared physical base and gives Data Fragments, exact Data Schematics, and hidden schematic-family unlockers three bold screen motifs. This preserves the exact vanilla silhouette, lighting, transparency, and blue-gray grading while keeping the motifs readable through the inventory's small mip levels. The transparent 128x128 production PNG sources live under `docs/icon-designs/production/`; the mod copies use vanilla-compatible BC7 sRGB DDS files with full mip chains.

The 18 vanilla files are maintenance snapshots, not a permanent fork of Keen's artwork. Before releases following a Space Engineers content update, run `tools/validate-working-knowledge-release.ps1` against the current game installation. The validator compares them with the installed vanilla copies and stops when a source was removed or changed so the definitions and artwork can be re-inspected. On systems without the game content available, it verifies that the bundled files exist and reports that the vanilla comparison was skipped. The three custom icons are checked for presence separately and are intentionally outside the vanilla-parity contract.
...
- Fragment entries are added idempotently during `LoadData`; if another source already added the same fragment item to a container type, Working Knowledge updates that entry's frequency instead of duplicating it.
