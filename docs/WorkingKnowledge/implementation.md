...
The vanilla set consists of the two terminal icons used by Working Knowledge's vanilla-derived terminal blocks and 16 cube icons used by the generated variant-group splits in `BlockVariantGroups_Research.sbc`. Keep their relative paths unchanged while they remain vanilla copies. The custom set gives Data Fragments, exact Data Schematics, and hidden schematic-family unlockers distinct silhouettes while retaining the subdued blue-gray visual language and 128x128 BC7 sRGB format of vanilla icons. Transparent production PNG sources live under `docs/icon-designs/production/`.
