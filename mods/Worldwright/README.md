...
Smoke is created directly through Space Engineers' particle system. Its two plume layers use the same neutral base color before the selected RGB multiplier is applied, preserving the full smoke volume without blending differently tinted layers. The spawner remains a normal TerminalBlock, so its terminal panel and toolbar actions are unaffected. Burst state is synchronized from the server for multiplayer clients.
